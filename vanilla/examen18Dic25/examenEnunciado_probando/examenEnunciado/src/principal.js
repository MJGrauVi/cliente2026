...
import {traerDatos} from "./js/otrasFunciones.js";
...
window.onload = async () => {
  ...
  const copiaLocal = cargarVJDesdeLocalStorage();
    console.log("copia local:", copiaLocal);
...
/*     const respuesta = await fetch("http://localhost:3000/videojuegos");
    ...
    const datos = await respuesta.json(); */
        ...
        const respuesta = await traerDatos("http://localhost:3000/videojuegos");
...
/*     const listaAdaptada = datos.map(vj => ({
  ...
  */
    ...
    renderTabla(seccionTablaBody, respuesta);
