...
const crearSelectorColor = () => {
  ...
  contenedor.addEventListener("click", (evento) => {
    if (evento.target.classList.contains("color")) {
      ...
      etiquetaColor.textContent = `Color seleccionado: ${colorSeleccionado.replace(
        "color-",
        ""
      )}`;
    }
...
export function activarPintura() {
  ...
  lienzo.addEventListener("pointermove", (e) => {
    ...
    if (
      objetivo &&
      objetivo.classList &&
      objetivo.classList.contains("celda")
    ) {
...
const configurarBotonReset = () => {
  ...
  boton.addEventListener("click", () => {
    ...
    etiqueta.textContent = `Color seleccionado: ${colorSeleccionado.replace(
      "color-",
      ""
    )}`;
  });
