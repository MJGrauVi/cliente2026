...
const validarFormulario = ({ autor, email, genero, fecha, pregunta }) => {
  let errores = [];
...
formulario.addEventListener("submit", (evento) => {
  ...
  const errores = validarFormulario({ autor, email, genero, fecha, pregunta });
  if (errores.length > 0) {
    mostrarErrores(errores);
    //No guardamos libro y cortamos ejecución.
    return;
  }
  discos = [...discos, { autor, email, genero, fecha, pregunta }];
...
const renderTablaDiscos = (lista) => {
  //Uso const global tabla.
  tabla.innerHTML = "";
  lista.map(({ autor, email, genero, fecha, pregunta }, index) => {
    //Creo la fila.
    const fila = document.createElement("tr");
    fila.innerHTML = `
                      <td>${autor}</td>
